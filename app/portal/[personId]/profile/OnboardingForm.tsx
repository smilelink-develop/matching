  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[28px] border border-white/80 bg-white/90 p-5 shadow-[0_14px_38px_rgba(15,23,42,0.08)]">
        <p className="text-sm font-semibold text-[#0F172A]">登録ステップ</p>
        <div className="mt-4 flex items-center gap-2">
          {[0, 1, 2].map((index) => (
            <div key={index} className="flex flex-1 items-center gap-2">
              <div className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-bold ${
                index <= step ? "bg-[#2563EB] text-white" : "bg-[#DBEAFE] text-[#2563EB]"
              }`}>
                {index + 1}
              </div>
              {index < 2 && (
                <div className={`h-1 flex-1 rounded-full ${index < step ? "bg-[#2563EB]" : "bg-[#DBEAFE]"}`} />
              )}
            </div>
          ))}
        </div>
      ...
      {step === 0 && (
        <section className="rounded-[28px] border border-white/80 bg-white/90 p-5 shadow-[0_14px_38px_rgba(15,23,42,0.08)]">
          <p className="text-sm font-semibold text-[#0F172A]">基本情報</p>
      ...
      {step === 1 && (
        <section className="rounded-[28px] border border-white/80 bg-white/90 p-5 shadow-[0_14px_38px_rgba(15,23,42,0.08)]">
          <p className="text-sm font-semibold text-[#0F172A]">住所</p>
          <div className="mt-4 space-y-4">
            <Field label="郵便番号">
      ...
      {step === 2 && (
        <section className="space-y-4">
          <section className="rounded-[28px] border border-white/80 bg-white/90 p-5 shadow-[0_14px_38px_rgba(15,23,42,0.08)]">
            <p className="text-sm font-semibold text-[#0F172A]">顔写真</p>
            <div className="mt-4 flex items-center gap-4">
              <AvatarPreview name={form.name} photoUrl={form.photoUrl} />
              <label className="inline-flex cursor-pointer items-center rounded-full bg-[#2563EB] px-4 py-2 text-sm font-medium text-white">
                写真を選択
          ...
          {form.documents.map((document) => (
            ...
            >
              ...
              <div className="mt-4 rounded-2xl border border-dashed border-[#BFDBFE] bg-[#F8FBFF] p-4">
                <p className="text-sm text-[#0F172A]">
                  {document.fileName ? `選択中: ${document.fileName}` : "まだ提出されていません"}
                </p>
                <label className="mt-3 inline-flex cursor-pointer items-center rounded-full bg-[#2563EB] px-4 py-2 text-sm font-medium text-white">
                  ファイルを選択
                  <input
                    type="file"
                    accept="image/*,.pdf"
      ...
      <div className="flex gap-3">
        {step > 0 && (
          <button
            ...
            className="flex-1 rounded-2xl border border-[#D6E4FF] bg-white px-4 py-3 text-sm font-medium text-[#2563EB]"
          >
            戻る
          </button>
        )}
        ...
        {step < 2 ? (
          <button
            type="button"
            onClick={goNext}
            className="flex-1 rounded-2xl bg-[#2563EB] px-4 py-3 text-sm font-medium text-white shadow-[0_12px_24px_rgba(37,99,235,0.2)]"
          >
            次へ
          </button>
        ) : (
          <button
            ...
            className="flex-1 rounded-2xl bg-[#2563EB] px-4 py-3 text-sm font-medium text-white shadow-[0_12px_24px_rgba(37,99,235,0.2)] disabled:opacity-50"
          >
...
function AvatarPreview({ name, photoUrl }: { name: string; photoUrl: string }) {
  ...
  return (
    <div className="flex h-[88px] w-[88px] items-center justify-center rounded-3xl bg-[#2563EB] text-3xl font-bold text-white">
      {name[0] ?? "人"}
    </div>
  );
}
...
const INPUT =
  "w-full rounded-2xl border border-[#D6E4FF] bg-[#F8FBFF] px-4 py-3 text-sm text-[#334155] outline-none";
