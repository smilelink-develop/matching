 ...
 */
export default function EditableIdBadge({ personId }: { personId: number }) {
    ...
    return (
      <span className="inline-flex items-center gap-1 text-[11px] text-gray-500">
        ...
        <input
          ...
          disabled={saving}
          className="w-20 rounded border border-gray-300 px-1 py-0.5 text-[12px] focus:outline-none focus:ring-1 focus:ring-[var(--color-primary)]/50"
          autoFocus
        />
        <button
          type="button"
          onClick={() => void submit()}
          disabled={saving}
          className="rounded bg-[var(--color-primary)] px-2 py-0.5 text-[11px] font-semibold text-white hover:bg-[var(--color-primary-hover)] disabled:opacity-50"
        >
          {saving ? "..." : "保存"}
        </button>
        <button
          ...
          onClick={cancel}
          disabled={saving}
          className="rounded border border-gray-300 px-2 py-0.5 text-[11px] text-gray-500 hover:bg-gray-50 disabled:opacity-50"
        >
          キャンセル
        </button>
        {error ? <span className="ml-1 text-[11px] text-red-500">⚠ {error}</span> : null}
      </span>
    );
  }
